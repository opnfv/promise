...
def main():
    ...
    neutron_client = os_utils.get_neutron_client(creds)

    nova_client = os_utils.get_nova_client(creds)

    glance_client = os_utils.get_glance_client(creds)
    ...
    temp_dir = os.path.join(CONST.dir_functest_data, "tmp/")
    images = { "image_name":"centos_7","image_url":"http://cloud.centos.org/centos/7/images/CentOS-7-x86_64-GenericCloud-1510.qcow2"}
    for image_name, image_url in images.iteritems():
        ...
        if image_id == '':
            ...
            image_id = os_utils.download_and_add_image_on_glance(
                glance_client, images["image_name"], images["image_url"], temp_dir)
    ...
    if args.report:
        stop_time = time.time()
        details = {"tests": int(tests), "failures": int(failures)}
        ...
        if int(tests) > 32 and int(failures) < 1:
            status = "PASS"
            exit_code = 0
            push_results("promise", start_time, stop_time, status, details)
